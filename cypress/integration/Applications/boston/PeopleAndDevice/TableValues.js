...
import writeJSON from '../../boston/Common/WriteJSON'
...
class TableValues{
    ...
    compareChartTotalWithTableTotal(dropdownID){
        cy.readFile('./cypress/fixtures/BpsPeopleAndDeviceChart.json').then((total) => {
    ...
    compareTableValuesWithStudentsTable(jsonName){
        cy.readFile('./cypress/fixtures/BpsPeopleAndDeviceStudentsTable.json').then((valueA) => {
