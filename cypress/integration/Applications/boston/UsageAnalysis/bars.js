...
class KPI{
    ...
    getKPINamesInaPage(jsonName,page,boxName){
        ...
        if(page>1){
            cy.writeFile('./cypress/fixtures/BpsUsageAnalysisKpis.json',' ,', { flag: 'a+' })
        }
        cy.xpath(boxName+KPI.bars).then((barsCount)=>{
            ...
            for(let i=1; i<=numberOfBars; i++){
                ...
                cy.xpath(kpiNamesXpath).invoke('text').then(kpiNames=>{
                    writing.writeValuesInArray('BpsUsageAnalysisKpis','"'+kpiNames+'"')
                })
                if(i<numberOfBars){
                    cy.writeFile('./cypress/fixtures/BpsUsageAnalysisKpis.json',' ,', { flag: 'a+' })
                }
    ...
    getTextInAllBars(boxName){
        const writing = new writeJson
        const jsonName = 'BpsUsageAnalysis'+boxName+'kpis'
        writing.start(jsonName)
        writing.startArray('BpsUsageAnalysisKpis',boxName)
        ...
        writing.endArray('BpsUsageAnalysisKpis')
        writing.end(jsonName)
    }
    checkBarNames(boxName){
        cy.readFile('./cypress/fixtures/BpsUsageAnalysisKpis.json').then((name)=>{
