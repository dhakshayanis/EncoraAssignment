class UsageVsAtAGlance{
    verifyStudentsInactive(){
        cy.readFile('./cypress/fixtures/SfpsTrackingAtAGlance.json').then((glance)=>{
            let expected = glance["DAILY INACTIVE STUDENTSAvg. Daily Inactive Students"][2]
            cy.readFile('./cypress/fixtures/SfpsUsageAnalysisStudentskpis.json').then((usage)=>{
    ...
    verifyTeachersActive(){
        cy.readFile('./cypress/fixtures/SfpsTrackingAtAGlance.json').then((glance)=>{
            ...
            cy.readFile('./cypress/fixtures/SfpsUsageAnalysisTeacherskpis.json').then((usage)=>{
