class UsageVsAtAGlance{
    verifyStudentsInactive(){
        cy.readFile('./cypress/fixtures/CfsTrackingAtAGlance.json').then((glance)=>{
            let expected = glance["DAILY INACTIVE STUDENTSAvg. Daily Inactive Students"][2]
            cy.readFile('./cypress/fixtures/CfsUsageAnalysisStudentskpis.json').then((usage)=>{
    ...
    verifyTeachersActive(){
        cy.readFile('./cypress/fixtures/CfsTrackingAtAGlance.json').then((glance)=>{
            ...
            cy.readFile('./cypress/fixtures/CfsUsageAnalysisTeacherskpis.json').then((usage)=>{
