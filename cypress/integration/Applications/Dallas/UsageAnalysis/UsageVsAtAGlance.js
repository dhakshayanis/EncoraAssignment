class UsageVsAtAGlance{
    verifyStudentsInactive(){
        cy.readFile('./cypress/fixtures/DisdTrackingAtAGlance.json').then((glance)=>{
            let expected = glance["DAILY INACTIVE STUDENTSAvg. Daily Inactive Students"][2]
            cy.readFile('./cypress/fixtures/DisdUsageAnalysisStudentskpis.json').then((usage)=>{
    ...
    verifyTeachersActive(){
        cy.readFile('./cypress/fixtures/DisdTrackingAtAGlance.json').then((glance)=>{
            ...
            cy.readFile('./cypress/fixtures/DisdUsageAnalysisTeacherskpis.json').then((usage)=>{
