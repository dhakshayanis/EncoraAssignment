class UsageVsAtAGlance{
    verifyStudentsInactive(){
        cy.readFile('./cypress/fixtures/SfusdTrackingAtAGlance.json').then((glance)=>{
            let expected = glance["DAILY INACTIVE STUDENTSAvg. Daily Inactive Students"][2]
            cy.readFile('./cypress/fixtures/SfusdUsageAnalysisStudentskpis.json').then((usage)=>{
    ...
    verifyTeachersActive(){
        cy.readFile('./cypress/fixtures/SfusdTrackingAtAGlance.json').then((glance)=>{
            ...
            cy.readFile('./cypress/fixtures/SfusdUsageAnalysisTeacherskpis.json').then((usage)=>{
