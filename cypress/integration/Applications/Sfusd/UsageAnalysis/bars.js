...
class KPI{
    ...
    getKPINamesInaPage(jsonName,page,boxName){
        ...
        if(page>1){
            cy.writeFile('./cypress/fixtures/SfusdUsageAnalysisKpis.json',' ,', { flag: 'a+' })
        }
        cy.xpath(boxName+KPI.bars).then((barsCount)=>{
            ...
            for(let i=1; i<=numberOfBars; i++){
                ...
                cy.xpath(kpiNamesXpath).invoke('text').then(kpiNames=>{
                    writing.writeValuesInArray('SfusdUsageAnalysisKpis','"'+kpiNames+'"')
                })
                if(i<numberOfBars){
                    cy.writeFile('./cypress/fixtures/SfusdUsageAnalysisKpis.json',' ,', { flag: 'a+' })
                }
    ...
    getTextInAllBars(boxName){
        const writing = new writeJson
        const jsonName = 'SfusdUsageAnalysis'+boxName+'kpis'
        writing.start(jsonName)
        writing.startArray('SfusdUsageAnalysisKpis',boxName)
        ...
        writing.endArray('SfusdUsageAnalysisKpis')
        writing.end(jsonName)
    }
    checkBarNames(boxName){
        cy.readFile('./cypress/fixtures/SfusdUsageAnalysisKpis.json').then((name)=>{
